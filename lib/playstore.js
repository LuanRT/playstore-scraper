  ...
  return new Promise((resolve, reject) => {
    ...
    .then((res) => {
      ...
      .each(function (i, el) {
        ...
        var icon = $($(utils.icon_selector)[i]).attr('data-srcset').slice(0, -2).trim();
        var link = `${playstore_base_url}${$(el).attr('href')}`;

        var rating = $($(utils.rating_selector)[i]).attr('aria-label')
        .replace("Rated", "")
...
const getExtendedInfo = (url) => {
  ...
  return new Promise((resolve,
    ...
    .then((res) => {
      ...
      var title = $($(utils.ext_title_selector)[0] || '<span>n/a</span>').text();

      var snippet = $(utils.ext_snippet_selector)[0]
      ? $($(utils.ext_snippet_selector)[0]).attr('content')
      .replace("\n", "")
      .trim(): "n/a";
      ...
      .replace(/<br>/g, "\n")
      .replace(/<\/br>|<b>/g, " ")
      .replace(/<\/b>|<p>|<\/p>|<i>|<\/i>|<\/div>|<i>|<\/i>|<h2>|<\/h2>/g, "");
      ...
      ? $($(utils.ext_icon_selector)[0]).attr('srcset').slice(0, -2).trim(): "n/a";

      var dev = $(utils.ext_dev_selector)[0]
      ? $(utils.ext_dev_selector)[0].children[0].data: "n/a";

      var dev_url = $(utils.ext_dev_selector)[0]
      ? `${playstore_base_url}${$($(utils.ext_dev_selector)[0]).attr('href')}`: "n/a";
      ...
      ? $($(utils.ext_ratings_selector)[0]).attr("aria-label"): "n/a";
      ...
      var genre = $($(utils.ext_genre_selector)[0] || '<span>n/a</span>').text();

      var price =
      $($(utils.ext_price_selector)[0]).attr('content') === "0"
      ? "Free": $($(utils.ext_price_selector)[0]).attr('content');

      var youtube_trailer = $(utils.ext_trailer_selector)[0]
      ? $($(utils.ext_trailer_selector)[0])
      .attr("data-trailer-url")
      .split("?")[0]
      .replace("https://www.youtube.com/embed/", "https://youtu.be/"): "n/a";
      ...
      */
      ...
      final_data.trailer = youtube_trailer.length === 0 ? 'n/a': youtube_trailer;
